...
require_once __DIR__ . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . 'configs' . DIRECTORY_SEPARATOR . 'cnf.php';

class YMEngine
{
    /**
     * @var const string    Constants with APIs urls.
     */
     ...
     * @throws \bogdanym\YMException        If params are not ok / curl could not be initialized.
     */
    public function __construct($strUserName, $strPassword)
    {
        /* verify username and password */
        if (!is_string($strUserName)) {
            throw new YMException('Invalid param username. Must be a string.');
        }
        $arrUser = explode('@', $strUserName);
        if (count($arrUser) > 2) {
            throw new YMException('Invalid param username. Must contain at most one @.');
        }
        if (!preg_match('/^[a-z0-9_\.\+]{1,32}$/i', $arrUser[0])) {
            throw new YMException('Invalid param username. ID must match [a-z0-9_.+] and must have at most 32 chars.');
        }
        if (isset($arrUser[1]) && !preg_match('/^[a-z0-9_\.\+]{3,64}$/i', $arrUser[1])) {
            throw new YMException('Invalid param username. DNS must match [a-z0-9_.+] and must have at most 64 chars.');
        }
        if (!is_string($strPassword) || strlen($strPassword) > 32) {
            throw new YMException('Invalid param password.');
        }
        ...
        if (!extension_loaded('curl')) {
            throw new YMException('cURL extension is not enabled.');
        }
        $this->curl = curl_init();
        if (false === $this->curl) {
            throw new YMException('cURL could not be initialized.');
        }
     ...
     * @throws \bogdanym\YMException                    If smth went wrong.
     */
    protected function makeApiCall(
        ...
        curl_setopt($this->curl, CURLOPT_USERAGENT, 'YahooMessenger/1.0 (' . APP_NAME . '; ' . APP_VERSION . ')');
        curl_setopt($this->curl, CURLOPT_SSL_VERIFYPEER, 0);
        
        $mxdResponse = curl_exec($this->curl);
        if (false === $mxdResponse) {
            throw new YMException(curl_error($this->curl), curl_errno($this->curl));
        }
    ...
    /**
     * Sets request token.
     * @return  \bogdanym\YMEngine
     * @throws  \bogdanym\YMException
     */
    public function fetchRequestToken()
    {
        $strUrl = self::URL_OAUTH_REQUEST_TOKEN
            ...
            . '&oauth_consumer_key=' . urldecode(APP_CONSUMER_KEY);
        ...
        if (2 != count($arrBody) || strtolower($arrBody[0]) != 'requesttoken') {
            throw new YMException('Could not get request token. Api response: ' . $strResponse);
        }
    ...
    public function fetchAccessToken($blnRenew = false)
    {
        ...
        if ($blnRenew) {
            if (!$this->hasAccessToken()) {
                throw new YMException('No access token to renew.');
            }
            $arrQuery['oauth_token'] = $this->tokens['access']['oauth_token'];
            $arrQuery['oauth_signature'] = APP_CONSUMER_SECRET . '&' . $this->tokens['access']['oauth_token_secret'];
            $arrQuery['oauth_session_handle'] = $this->tokens['access']['oauth_session_handle'];
            $this->setTokenRenewed(true);
        } else {
            if (!$this->hasRequestToken()) {
                throw new YMException('No request token previously set.');
            }
            $arrQuery['oauth_token'] = $this->tokens['request'];
            $arrQuery['oauth_signature'] = APP_CONSUMER_SECRET . '&';
        }
        ...
        if (count($arrBody) < 4
            ...
            throw new YMException('Could not get access token. Api response: ' . $strResponse);
        }
    ...
    public function logIn($intStatus = self::USER_IS_ONLINE)
    {
        ...
        if (200 != $intHttpStatus) {
            throw new YMException('Could not create session. Api response: ' . $strResponse);
        }
    ...
    public function checkSession()
    {
        ...
        if (200 != $intHttpStatus) {
            throw new YMException('Could not check session. Api response: ' . $strResponse);
        }
    ...
    public function keepAliveSession()
    {
        ...
        if (200 != $intHttpStatus || trim($strResponse) != '') {
            throw new YMException('Could not keepalive session. Api response: ' . $strResponse);
        }
    ...
    public function logOut()
    {
        ...
        if (200 != $intHttpStatus || trim($strResponse) != '') {
            throw new YMException('Could not log out. Api response: ' . $strResponse);
        }
    ...
    public function fetchCustomAvatar($strUserId, $strNetwork = 'yahoo', $strImgSize = 'small', $strImgFormat = 'png')
    {
        ...
        if (200 != $intHttpStatus) {
            throw new YMException('Could not get user avatar. Api response: ' . $strResponse);
        }
    ...
    public function fetchGroups()
    {
        ...
        if (200 != $intHttpStatus) {
            throw new YMException('Could not fetch groups. Api response: ' . $strResponse);
        }
...
//            }
        ...
        if (200 != $intHttpStatus) {
            throw new YMException('Could not fetch notifications. Api response: ' . $strResponse);
        }
     ...
     * @param  string   $strNetwork     The user 's network.
     * @return \bogdanym\YMEngine
     * @throws \bogdanym\YMException
     */
    public function sendMessage($strMsg, $strUserId, $strNetwork = 'yahoo')
    {
        ...
        if (200 != $intHttpStatus || trim($strResponse) != '') {
            throw new YMException('Could not send message. Api response: ' . $strResponse);
        }
        return $this;
    }
    ...
    public function changePresenceState($intState, $strMessage)
    {
        ...
        if (200 != $intHttpStatus || trim($strResponse) != '') {
            throw new YMException('Could not change presence. Api response: ' . $strResponse);
        }
        return $this;
    }
    ...
    public function authorizeBuddy(
        ...
        if (200 != $intHttpStatus || trim($strResponse) != '') {
            throw new YMException('Could not authorize buddy. Api response: ' . $strResponse);
        }
    ...
    {
        if (!isset($this->tokens['access']['oauth_token_secret']) || !isset($this->tokens['access']['oauth_token'])) {
            throw new YMException('No access token previously set.');
        }
        return 'Authorization: OAuth realm="yahooapis.com",'
            . 'oauth_nonce="' . uniqid(mt_rand(1, 1000)) . '",'
            . 'oauth_consumer_key="' . APP_CONSUMER_KEY . '",'
            . 'oauth_signature_method="PLAINTEXT",'
            . 'oauth_signature="' . urlencode(APP_CONSUMER_SECRET . '&' . $this->tokens['access']['oauth_token_secret']) . '",'
            . 'oauth_timestamp="' . time() . '",'
            . 'oauth_version="1.0",'
